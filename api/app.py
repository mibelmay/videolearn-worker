...
from src.questions_generating.generate_questions import generate_questions_with_gigachat
...
async def generate_questions(req: VideoRequest):
    ...
    questions = await generate_questions_with_gigachat(transcription, req.is_pro_user)
    return questions

@app.post("/api/get-transcription")
async def generate_questions(req: VideoRequest):
    loop = asyncio.get_event_loop()
    transcription = await loop.run_in_executor(None, transcribe_video, req.video_url, req.language)
